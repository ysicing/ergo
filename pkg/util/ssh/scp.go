...
func (ss *SSH) CopyForMD5(host, localFilePath, remoteFilePath, md5 string) bool {
	...
	ss.Log.Infof("source file md5 value is %s", md5)
	ss.Copy(host, localFilePath, remoteFilePath)
	remoteMD5 := ss.Md5Sum(host, remoteFilePath)
	ss.Log.Infof("host: %s , remote md5: %s", host, remoteMD5)
	remoteMD5 = strings.TrimSpace(remoteMD5)
	md5 = strings.TrimSpace(md5)
	if remoteMD5 == md5 {
		ss.Log.Infof("md5 validate true")
		return true
	}
	ss.Log.Warn("[ssh]md5 validate false")
	return false
}
...
func (ss *SSH) Copy(host, localFilePath, remoteFilePath string) {
	...
	for {
		...
		ss.Log.Debugf("[%s]transfer total size is: %.2f%s ;speed is %d%s", host, totalLength, totalUnit, speed, unit)
	}
}
...
func (ss *SSH) CopyConfigFile(host, remoteFilePath string, localFilePathOrBytes interface{}) {
	...
	for {
		...
		ss.Log.Debugf("[%s]transfer total size is: %d%s", host, totalMB, "MB")
	}
}
...
func (ss *SSH) copyLocalFileToRemote(host string, sshClient *ssh.Client, sftpClient *sftp.Client, localPath, remotePath string) {
	...
	if err != nil {
		ss.Log.Errorf("sftp create err: %v", err)
	}
	defer dstFile.Close()
	buf := make([]byte, 100*oneMBByte) //100mb
	total := 0
	...
	if !ss.isCopyMd5Success(sshClient, localPath, remotePath) {
		//	ss.Log.Infof("[%s] copy local file: %s to remote file: %s validate md5sum success", host, localPath, remotePath)
		//} else {
		ss.Log.Debugf("[%s] copy local file: %s to remote file: %s validate md5sum failed", host, localPath, remotePath)
	}
}
