...
func CleanPackage(user, token string) {
	...
	if err != nil {
		log.Panicf("list package err: %v", err)
	}
	for _, p := range packages {
		log.Debugf("package %v", p.GetName())
		...
		if err != nil {
			log.Debugf("list package version err: %v, skip", err)
			continue
		}
		for _, pv := range packagesversions {
			clean(ctx, client, pv, log, p.GetOwner().GetName(), p.GetName())
		}
	}
}

func clean(ctx context.Context, client *github.Client, p *github.PackageVersion, log log.Logger, user, name string) {
