...
func NewDB(dsn string) (*DB, error) {
	...
	if err != nil {
		zlog.Errorf("create sql client err: %v", err)
		return nil, err
	}
...
func (db *DB) Drop(dbname, dbuser string) error {
	...
	db.zlog.Debugf("delete db %v", dbname)
	_, err = db.client.Exec("flush privileges;")
	if err != nil {
		db.zlog.Errorf("flush privileges err: %v", err)
		return err
...
func (db *DB) Close() {
	if err := db.client.Close(); err != nil {
		db.zlog.Errorf("close db err: %v", err)
	}
}
